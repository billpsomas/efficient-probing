...
from .clip.conv_bn_act import ConvNormAct
from .clip.create_act import create_act_layer, get_act_layer
from .clip.helpers import make_divisible
...
class CbamPooling(nn.Module):
    def __init__(
            self, channels, rd_ratio=1./16, rd_channels=None, rd_divisor=1,
            spatial_kernel_size=7, act_layer=nn.ReLU, gate_layer='sigmoid', mlp_bias=False, output_size=1):
        super(CbamPooling, self).__init__()
        self.channel = ChannelAttn(
            channels, rd_ratio=rd_ratio, rd_channels=rd_channels,
            rd_divisor=rd_divisor, act_layer=act_layer, gate_layer=gate_layer, mlp_bias=mlp_bias)
        self.spatial = SpatialAttn(spatial_kernel_size, gate_layer=gate_layer)
    ...
    def forward(self, x):
        ...
        ## NEW FOR TRANSFORMER OUTPUT ##
        B, N, C = x.shape
        
        H = W = int(N ** 0.5)
        assert H * W == N, "n must be a perfect square for reshaping."

        # Permute the tensor from (b, n, d) to (b, d, n)
