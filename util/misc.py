...
def init_distributed_mode(args):
    if args.dist_url == "no-dist":
        print('Not using distributed mode (single GPU detected)')
        setup_for_distributed(is_master=True)  # Hack to enable non-distributed logging
    ...
    if args.dist_on_itp:
        ...
        print("rank", os.environ['RANK'], os.environ['WORLD_SIZE'])
        args.rank = int(os.environ["RANK"])
        args.world_size = int(os.environ['WORLD_SIZE'])
        args.gpu = int(os.environ['LOCAL_RANK'])
    elif 'SLURM_PROCID' in os.environ:
        print("slurm", os.environ['SLURM_PROCID'])
    ...
    torch.distributed.init_process_group(backend=args.dist_backend, init_method=args.dist_url,
                                         world_size=args.world_size, rank=args.rank, timeout=timedelta(hours=1))
...
def load_model(args, model_without_ddp, optimizer=None, loss_scaler=None, strict=True):
    if args.resume:
        ...
        if optimizer is not None:
            if 'optimizer' in checkpoint and 'epoch' in checkpoint and not (hasattr(args, 'eval') and args.eval) and not (hasattr(args, 'knn_eval') and args.knn_eval):
